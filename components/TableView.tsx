...
import { TableData, Row, Column, ColumnType, SortState, Filter, TextOverflowMode } from '../types';
    ...
    return (
        <div className={`relative ${className}`} ref={containerRef}>
            <button
                ...
                className="w-full flex items-center justify-between px-3 py-2 bg-white border border-gray-200 rounded-md text-sm text-[#323232] hover:bg-gray-50 transition-colors focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none h-9"
            >
                <span className="truncate">{selectedLabel}</span>
                <IconChevronRight className={`w-3 h-3 text-gray-400 transition-transform ${isOpen ? 'rotate-90' : 'rotate-0'}`} />
            </button>
            
            {isOpen && (
                <div className="absolute top-full left-0 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-48 overflow-y-auto z-50 animate-in fade-in zoom-in-95 duration-100">
                    {options.map(opt => (
                        <button
                            key={opt.value}
                            onClick={() => { onChange(opt.value); setIsOpen(false); }}
                            className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 flex items-center justify-between ${value === opt.value ? 'bg-blue-50 text-blue-600 font-medium' : 'text-[#323232]'}`}
                        >
                            {opt.label}
...
export const TableView: React.FC<TableViewProps> = ({ 
    ...
    useEffect(() => {
        if (showGenPanel) {
            ...
            setGenSelectedColIds(new Set(table.columns
    ...
    useEffect(() => {
        ...
        if (table.rows.length < MIN_ROWS) {
           ...
           for(let i=0; i<needed; i++) {
               const r: Row = { id: `empty_${Date.now()}_${i}` };
    ...
    const handleUnifiedDelete = () => {
        if (selectedRowIds.size > 0) {
            if (confirm(`選択された ${selectedRowIds.size} 件の行を削除しますか？`)) {
                onUpdateTable(prev => ({
                    ...prev,
    ...
    const handleDeleteColumn = (colId: string) => {
        if (confirm('このカラムを削除しますか？データは失われます。')) {
            onUpdateTable(prev => ({
                ...prev,
    ...
    const handleGenerateStart = async () => {
        ...
        if (genNewColsString.trim()) {
            ...
            currentColumns = [...currentColumns, ...newColsCreated];
        ...
        try {
            const generatedRows = await generateRows(columnsToGenerate, genCount, genPrompt || `既存のデータと同様のもの`);
            onUpdateTable(prev => {
                const newRows = [...prev.rows];
    ...
    const handleEnrichmentStart = async () => {
        ...
        selectedRowIds.forEach(rowId => {
            ...
            targetCols.forEach(col => {
                const task = (async () => {
                    try {
                        const val = await enrichRowData(row, col, otherCols);
                            ...
                            <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-gray-200 shadow-xl rounded-lg p-0 z-50 animate-in fade-in zoom-in-95 duration-100">
                                ...
                                <div className="p-3 space-y-3 max-h-60 overflow-y-auto">
                                    ...
                                    {activeFilters.map((f, idx) => (
                                        <div key={idx} className="flex items-center gap-2 text-xs bg-gray-50 p-2 rounded border border-gray-100">
                                            <span className="font-bold text-[#323232]">{table.columns.find(c => c.id === f.columnId)?.title}</span>
                                ...
                                <div className="p-3 border-t border-gray-100 bg-gray-50/50 space-y-2">
                                    ...
                                    <CustomSelect 
                                        ...
                                        options={table.columns.map(c => ({ value: c.id, label: c.title }))}
                                    />
                                ...
                                <div className="p-3 space-y-3 max-h-60 overflow-y-auto">
                                    ...
                                    {activeSorts.map((s, idx) => (
                                        <div key={idx} className="flex items-center gap-2 text-xs bg-gray-50 p-2 rounded border border-gray-100">
                                            ...
                                            <span className="font-bold text-[#323232]">{table.columns.find(c => c.id === s.columnId)?.title}</span>
                        ...
                        {showEnrichPanel && (
                            <div className="absolute right-0 top-full mt-2 w-72 bg-white border border-gray-200 shadow-xl rounded-lg overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-100 origin-top-right">
                                ...
                                <div className="p-2 max-h-60 overflow-y-auto">
                                    {table.columns.filter(col => !isPlaceholderColumn(col)).map(col => (
                                ...
                                >
                                    <div className="flex flex-col h-full">
                                        ...
                                        >
                                            ...
                                            <span className="font-semibold text-xs tracking-wide text-[#323232] truncate flex-1">{col.title}</span>
                                ...
                                >
                                    ...
                                    {table.columns.map(col => {
                                        ...
                                        if (typeof rawValue === 'string' && rawValue.startsWith('=')) {
                                            displayValue = evaluateFormula(rawValue, row, table.columns);
                                        }
